...
var JeelizSVGHelper=(function(){
    ...
    function onMorphUpdate(quality, benchmarkCoeff){
        _morphIndexToName.forEach(function(morphKey, morphIndex){
            ...
            that._output.data[morphKey] = _morphFactorsArr[morphIndex];
        });
    ...
    var that={
        ...
        _output: {
            ...
            do_output: function(){
                ...
                var text = "";
                var canvas = document.getElementById("digitalLabsOutputCanvas");
                var ctx = canvas.getContext("2d");
                ctx.font = "15px Arial";
                ctx.fillStyle = "#cccccc";
                ctx.fillRect(10, 0, 250, (55 * items.length) );
                ctx.fillStyle = "#000000";


                for (var i = 0; i<items.length; i++ ){
                    var item = items[i];
                    var value = this.data[item].toPrecision(2);
                    text =  item + " : " + value;
                    ctx.fillText(text, 13, 15 + (18 * i));
                }
